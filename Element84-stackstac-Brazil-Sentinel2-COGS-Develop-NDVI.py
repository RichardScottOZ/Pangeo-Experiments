...
#bbox = [-53.0232820986343754,-8.1236837545427090, -52.9688521093868800,-7.9677173521785928] #carra grav
...
##https://sentinel-cogs.s3.us-west-2.amazonaws.com/sentinel-cogs/sentinel-s2-l2a-cogs/22/M/FV/2020/9/S2A_22MFV_20200902_0_L2A/B02.tif

#stack fixes - error

#'/vsicurl/https://sentinel-cogs.s3.us-west-2.amazonaws.com/sentinel-s2-l2a-cogs/22/M/FV/2020/9/S2A_22MFV_20200902_0_L2A/B02.tif  #new one 20210506
...
print("CROPPED CLEAR POST NDVI\n\n", cropped_clear)

#cropped_clear = cropped_clear.where(cropped_clear.NDVI <= cropped_clear.NDVI.min() )
cropped_clear_ndvi = cropped_clear["NDVI"].min(dim="time", skipna=True)
print("CROPPED CLEAR NDVI\n\n", cropped_clear_ndvi)

print(cropped_clear)

cropped_clear = cropped_clear.where( cropped_clear["NDVI"] <= cropped_clear_ndvi, drop=True  )
...
if 1 == 1:
    with dask.diagnostics.ProgressBar():
        ...
        croppedNP.rio.to_raster(r'F:\Brazil\sentinel_median_carra_grav_20000_0.tif')
